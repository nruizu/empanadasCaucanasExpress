...
class TwilioNotificationService:
    ...
    def __init__(self):
        ...
        if self._client is None and self._has_twilio_credentials():
            try:
                ...
                logger.warning("Twilio no está instalado. Las notificaciones estarán deshabilitadas.")
    ...
    def send_order_confirmation(
        ...
        Args:
            phone_number (str): Número de teléfono del cliente (formato: +573001234567)
            order_id (int): ID del pedido
            customer_name (str): Nombre del cliente
            delivery_method (str): Modalidad de entrega (pickup, delivery, scheduled)
        ...
        Returns:
            Tuple[bool, Optional[str], Optional[str]]: (éxito, message_sid, error_message)
        """
        if not self._client:
            logger.warning(
                f"Twilio no configurado. No se envió notificación para pedido {order_id}"
            )
    ...
    def send_order_status_update(
        ...
        if not self._client:
            logger.warning(
                f"Twilio no configurado. No se envió actualización para pedido {order_id}"
            )
            return False, None, "Twilio no está configurado"
...
Empanadas Caucanas Express 🤤"""
    ...
    def _build_status_update_message(
        ...
        """Construye el mensaje de actualización para estados confirmación, preparación y listo."""
        status_map = {
