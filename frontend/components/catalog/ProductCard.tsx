      ...
      <div className="flex min-h-44 flex-col justify-between p-4">
        <div>
          ...
          {product.variants.length > 1 ? (
            <div className="mt-3">
              <label htmlFor={`variant-${product.id}`} className="mb-1 block text-xs font-semibold text-[var(--cce-text-muted)]">
                Presentación
              </label>
              <select
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <span className="text-lg font-bold text-[var(--cce-green-dark)]">{formatPrice(selectedVariant.price)}</span>
          <AddToCartButton
